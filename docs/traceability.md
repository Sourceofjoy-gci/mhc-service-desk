...
|---|---|---|---|
| Authentication and staff shell | `identity_access.authentication.KeycloakJWTAuthentication`; `GET /api/v1/identity/me`; `AuthProvider` | `backend/apps/identity_access/tests/test_authentication.py`; `frontend/src/features/auth/AuthProvider.test.tsx`; `frontend/src/app/App.test.tsx` | Backend pytest/mypy and frontend gates passed; clean-checkout Ruff remains open |
| Scope separation and restricted access (FR-026, FR-027) | `identity_access.scope`; scoped ticket/file/report querysets | `identity_access/tests/test_scope.py`; `tickets/tests/test_scope_api.py`; `files/tests/test_views.py`; `reporting/tests/test_permissions.py` | Backend pytest/mypy and permission audit passed; clean-checkout Ruff remains open |
| Assignment and work state (FR-033) | `PATCH /api/v1/tickets/{number}/work-state/`; `GET .../assignees/` | `tickets/tests/test_work_state_api.py`; `tickets/tests/test_permissions.py`; `frontend/src/features/tickets/OperationsPanel.test.tsx` | Backend pytest/mypy and frontend gates passed; clean-checkout Ruff remains open |
| Workflow and role-gated transitions (FR-038, FR-040) | `POST /api/v1/tickets/{number}/transition/`; `tickets.workflow.available_transitions` | `tickets/tests/test_transition_api.py`; `tickets/tests/test_workflow_capabilities.py`; `frontend/src/features/tickets/TransitionActions.test.tsx` | Backend pytest/mypy, live smoke, and frontend gates passed; clean-checkout Ruff remains open |
...
| Queue, Kanban, URL filters (FR-039, FR-041, FR-043, FR-081, FR-082) | `GET /api/v1/tickets/`; `GET /api/v1/tickets/kanban/`; `QueuePage`; `KanbanPage` | `tickets/tests/test_api_collections.py`; `frontend/src/features/tickets/QueuePage.test.tsx`; `KanbanPage.test.tsx` | Backend pytest/mypy and frontend gates passed; clean-checkout Ruff and rendered browser checks remain open |
...
| Internal notes and requester replies (FR-015, FR-016) | `GET/POST .../messages/`; `GET/POST .../notes/`; `GET .../activity/` | `tickets/tests/test_activity.py`; `frontend/src/features/tickets/ActivityTimeline.test.tsx`; `MessageComposer.test.tsx` | Backend pytest/mypy, live smoke, and frontend gates passed; clean-checkout Ruff remains open |
| SLA state/display (FR-052-FR-059) | `sla.services`; `sla.serializers.serialize_sla_clocks`; ticket `sla_clocks` | `sla/tests/test_services.py`; `sla/tests/test_serializers.py`; `frontend/src/features/tickets/SlaClocks.test.tsx` | Backend pytest/mypy and frontend gates passed; clean-checkout Ruff and operator policy validation remain open |
| Attachments (FR-017, FR-093-FR-095) | `GET/POST /api/v1/tickets/{number}/attachments/`; `GET /api/v1/attachments/{id}/download/` | `files/tests/test_views.py`; `files/tests/test_events.py`; `frontend/src/features/tickets/AttachmentUploader.test.tsx` | Backend pytest/mypy and frontend gates passed; clean-checkout Ruff, browser checks, and production object-store/ClamAV exercise remain open |
| Ticket relationships and IT child (FR-019, FR-028-FR-030) | `POST .../it-child/`; `tickets.it_child`; scoped relationships | `tickets/tests/test_it_child.py`; relationship tests in `tickets/tests/test_activity.py` | Backend pytest/mypy and live smoke passed; clean-checkout Ruff remains open |
...
| 8 | SLA business-calendar cases | `sla/tests/test_services.py` and serializer tests | Backend pytest/mypy passed; clean-checkout Ruff and operator policy validation remain |
| 9 | Email threading/idempotency | `email_channel/tests/test_services.py`; legacy M3 smoke | Backend pytest/mypy passed; clean-checkout Ruff and provider testing remain |
...
| 12 | Files are external and clean-only | File service/view tests and AttachmentUploader tests | Backend pytest/mypy passed; clean-checkout Ruff and production object-store/ClamAV exercise remain |
