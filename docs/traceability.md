...
| Workflow and role-gated transitions (FR-038, FR-040) | `identity_access.staff_roles`; `POST /api/v1/tickets/{number}/transition/`; `GET /api/v1/tickets/{number}/escalation-supervisors/`; `tickets.services.transition_ticket`; `tickets.workflow.available_transitions`; `TransitionActions` | `tests/test_primary_staff_roles.py`; `tickets/tests/test_seed_workflow_roles.py`; `tickets/tests/test_transition_api.py`; `tickets/tests/test_workflow_capabilities.py`; `tickets/tests/test_integrity_boundaries.py`; `tickets/tests/test_escalation_service.py`; `tickets/tests/test_escalation_api.py`; `frontend/src/features/tickets/TransitionActions.test.tsx` | Deputy Master has Master-equivalent Operational approval authority only within the Deputy's active persisted exact scope; Assistant Master and scope, expiry, confidentiality, domain, and auditor boundaries remain enforced. Named operational supervisor selection, atomic escalation ownership/evidence, and the preserved IT reason-only path are covered; clean-checkout Ruff and live role-based release verification remain open |
